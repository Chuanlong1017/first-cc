              ...
              if (child.material) {
                if (Array.isArray(child.material)) {
                  if (child.material[0] && child.material[0].color) {
                    matColor.copy(child.material[0].color);
                  }
                } else if (child.material.color) {
                  matColor.copy(child.material.color);
                }
              }
              const count = geo.attributes.position.count;
              const colors = new Float32Array(count * 3);
        ...
        fbx.traverse(child => {
          if (child.isMesh && child.geometry) {
            ...
            if (!geo.attributes.color && child.material) {
              ...
              if (child.material.color) matColor.copy(child.material.color);
              const count = geo.attributes.position.count;
              const colors = new Float32Array(count * 3);
              for (let i = 0; i < count; i++) {
                colors[i * 3] = matColor.r;
